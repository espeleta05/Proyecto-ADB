-- datos para PostgreSQL (usar con esquemaBD.psql)
...
-- workers (incluye ids usados en vaccination_records)
INSERT INTO workers (worker_id, name, lastname, role, mail, curp, address, birth_date, password_hash, created_at) VALUES
(3,'Enfermero','Tres','Enfermero','enf3@mail.com','CURPW3','MTY','1992-03-03','$2b$12$f4jJCWwqo7Wv1qXiCFBUsOYz5tHT09pffJS9UMy44NZ1hJ5.HnSay',NOW()),
(4,'Enfermero','Cuatro','Enfermero','enf4@mail.com','CURPW4','MTY','1993-04-04','$2b$12$f4jJCWwqo7Wv1qXiCFBUsOYz5tHT09pffJS9UMy44NZ1hJ5.HnSay',NOW())
...
(8,'DPT',90,'Sanofi','Difteria, tos ferina y tetanos (refuerzo)'),
...
ON CONFLICT (id_vaccine) DO NOTHING;
...
(3, 'Primera Dosis', 2, 0),
(3, 'Segunda Dosis', 4, 56),
(3, 'Tercera Dosis', 6, 56),
(3, 'Cuarta Dosis', 18, 365),
(4, 'Primera Dosis', 2, 0),
(4, 'Segunda Dosis', 4, 28),
(4, 'Tercera Dosis', 6, 28),
(5, 'Primera Dosis', 2, 0),
(5, 'Segunda Dosis', 4, 56),
(5, 'Refuerzo', 12, 180),
(6, 'Primera Dosis', 6, 0),
(6, 'Segunda Dosis', 7, 28),
...
(7, 'Segunda Dosis', 72, 365),
(8, 'Refuerzo', 48, 0),
(9, 'Unica', 59, 0),
(12, 'Unica', 112, 0)
ON CONFLICT DO NOTHING;

-- registros de vacunacion por paciente
INSERT INTO vaccination_records (record_id, patient_id, vaccine_id, worker_id, applied_date, dose_applied, lot_number) VALUES
(1,1,1,3,'2024-01-01','Primera','LOT001'),
(2,2,2,3,'2024-01-02','Primera','LOT002'),
(3,3,3,4,'2024-01-03','Primera','LOT003'),
(4,4,4,4,'2024-01-04','Primera','LOT004'),
(5,5,5,3,'2024-01-05','Primera','LOT005'),
(6,6,6,4,'2024-01-06','Primera','LOT006'),
(7,7,7,3,'2024-01-07','Primera','LOT007'),
(8,8,8,4,'2024-01-08','Primera','LOT008'),
(9,9,9,3,'2024-01-09','Primera','LOT009'),
(10,10,10,4,'2024-01-10','Primera','LOT010')
ON CONFLICT (record_id) DO NOTHING;

-- sincronizar secuencias identity despues de insertar ids manuales
SELECT setval(pg_get_serial_sequence('patients', 'patient_id'), COALESCE((SELECT MAX(patient_id) FROM patients), 1), true);
SELECT setval(pg_get_serial_sequence('guardians', 'guardian_id'), COALESCE((SELECT MAX(guardian_id) FROM guardians), 1), true);
SELECT setval(pg_get_serial_sequence('relations', 'relation_id'), COALESCE((SELECT MAX(relation_id) FROM relations), 1), true);
SELECT setval(pg_get_serial_sequence('workers', 'worker_id'), COALESCE((SELECT MAX(worker_id) FROM workers), 1), true);
SELECT setval(pg_get_serial_sequence('vaccines', 'id_vaccine'), COALESCE((SELECT MAX(id_vaccine) FROM vaccines), 1), true);
SELECT setval(pg_get_serial_sequence('vaccination_scheme', 'id_scheme'), COALESCE((SELECT MAX(id_scheme) FROM vaccination_scheme), 1), true);
SELECT setval(pg_get_serial_sequence('vaccination_records', 'record_id'), COALESCE((SELECT MAX(record_id) FROM vaccination_records), 1), true);
